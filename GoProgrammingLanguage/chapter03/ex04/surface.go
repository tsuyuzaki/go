/**
 * 1,7節のリサジューの例での方法に従って、面を計算して SVG データをクライアントに書き出すウェブサーバを作成しなさい。
 * サーバは次のように Content-Type を設定しなければなりません。
 * 
 * w.Header.Set("Content-Type", "image/svg+xml")
 * 
 * (このステップはリサジューの例では必要ありませんでした。
 *  それは、サーバが標準的なヒューリスティクスを使ってレスポンスの最初の 512 バイトから PNG などの共通形式を認識し、
 *  適切なヘッダーを生成しているからです。)
 * HTTPリクエストのパラメータとして、高さ、幅、色などの値をクライアントが指定できるようにしなさい。
 * 
